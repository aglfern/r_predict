...
load_incidentevtlog <- function(dirName, fileName, numberOfRows, fileVersion) {
   ...
   if ( fileVersion == 1 )
   {
      ...
      # outras colunas específicas desse arquivo
   ...
   # logica para usar o ultimo valor quando há mais de um registro de status
   # closed e resolved
   ...
   #seleciona apenas incidentes que  estão concluidos status == closed
   # os demais não servem para o modelo
   ...
   # replica tempo de conclusão para todos os registros das variaveis target
...
#   incidentevtlog[evt_with_na_mod_count,]$sys_updated_by <- 'system'
   ...
   # cria campos com valor inteiro para a contagem do tempo (nova versão, mais genérica)
   ...
   # elapsed: opção 1 = updated - opened; opção 2 = updated - created
   ...
   # remaining: opção 1 = closed - updated; opção 2 = resolved - updated
   ...
   #em execução: opção 1,1


   # tempo de sojourn a ser preenchido na construção do modelo
